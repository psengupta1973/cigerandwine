...
db = new Db('cigarandwinedb', server, {safe: true});

db.open(function(err, db) {
    if(!err) {
        console.log("Connected to 'cigarandwinedb' database");
		populateDBWines();
		populateDBCigars();
...
exports.findCigarById = function(req, res) {
    var id = req.params.id;
...
exports.findAllCigars = function(req, res) {
    db.collection('cigars', function(err, collection) {
        collection.find().toArray(function(err, items) {
            res.send(items);
        });
